...
- GET /:version/apps/:id/apppsp/ → Gets a collection of all (but always one) App Payment Service Provider. Combination of webshop:id and App:id is unique.
- GET /:version/apppsp/:id/ → Gets on App Payment Service Provider by Resource ID.
- GET /:version/apppsp/ → Gets a collection of all (but always one) App Payment Service Provider. Combination of webshop:id and App:id is unique.
- OPTIONS /:version/apppsp/ → None
...
class Apppsp(CCVApiEndpoints):
    ...
    def gets_a_collection_of_all_(but_always_one)_app_payment_service_provider_combination_of_webshop:id_and_app:id_is_unique(
        self,
        id: str,
        per_page: int = 100,
        total_pages: int = 1,
        **params,
        ...
        """
        Gets a collection of all (but always one) App Payment Service Provider. Combination of webshop:id and App:id is unique.
        """
